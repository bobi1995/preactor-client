...
import { useDeleteChangeoverGroup } from "../../graphql/hook/changeover";
...
const DeleteChangeoverGroupDialog: React.FC<Props> = ({ group }) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const { deleteChangeoverGroup, loading, error } = useDeleteChangeoverGroup();

  const handleDelete = async () => {
    try {
      const result = await deleteChangeoverGroup(group.id);
      if (result) {
        setIsOpen(false);
        toast.success(t("changeoverGroupsPage.deleteToast"));
      }
    } catch (err) {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 max-w-md w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-lg z-50 animate-in zoom-in-95">
          <Dialog.Title className="text-lg font-semibold">
            {t("changeoverGroupsPage.deleteDialog.title", "Delete Group")}
          </Dialog.Title>
          ...
          {loading ? (
            ...
            <Dialog.Description className="mt-2 text-sm text-gray-600">
              {t("changeoverGroupsPage.deleteDialog.description", {
                name: group.name,
                defaultValue: `Are you sure you want to delete "${group.name}"? This action cannot be undone.`,
              })}
            </Dialog.Description>
          )}
...
export default DeleteChangeoverGroupDialog;
