...
import { useCreateChangeoverGroup } from "../../graphql/hook/changeover";
...
const CreateChangeoverGroupDialog: React.FC<Props> = ({ allGroups }) => {
  const { t } = useTranslation();
  const [name, setName] = useState("");
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const { createChangeoverGroup, loading } = useCreateChangeoverGroup();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (
      allGroups.some((g) => g.name.toLowerCase() === name.trim().toLowerCase())
    ) {
      setValidationError({
    ...
    try {
      await createChangeoverGroup(name);
      setIsDialogOpen(false);
      toast.success(t("changeoverGroupsPage.createToast"));
    } catch (err) {
      setValidationError({
        message: t("errors.errorGeneral"),
        key: Date.now(),
      });
    }
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl max-w-md w-[90vw] p-6 z-50 animate-in zoom-in-95">
          <Dialog.Title className="text-xl font-semibold mb-1 text-slate-800">
            {t("changeoverGroupsPage.createDialog.title", "Create Group")}
          </Dialog.Title>
          <Dialog.Description className="mb-5 text-sm text-slate-500">
            {t(
              "changeoverGroupsPage.createDialog.description",
              "Add a new changeover group."
            )}
          </Dialog.Description>
...
export default CreateChangeoverGroupDialog;
